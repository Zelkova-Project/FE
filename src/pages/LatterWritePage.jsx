        ...
        <div className={style["container"]}>
          ...
          <div className={style["write-item"]}>
            <div className={style["write-txt"]}>
              <label>
                <h3>게시 여부</h3>
                <input type={'text'} placeholder={'공개'} value={activitySelectVal} autoComplete={'off'}
                       className={style['activity-select']} onClick={() => {
                  setActivitySelected(!activitySelected);
                }}/>
                <img src={imgObj.select} alt={'select'} className={style['activity-select-img']} onClick={() => {
                  setActivitySelected(!activitySelected);
                }}/>
                {activitySelected ? (
                    <ul className={style['activity-option']}>
                      <li className={style['option']} onClick={() => activitySelectValue('공개')}>공개</li>
                      <li className={style['option']} onClick={() => activitySelectValue('비공개')}>비공개</li>
                    </ul>
                ) : null}
              </label>
            </div>
          ...
          <div className={[style['write-item'], style['mt-90']].join(' ')}>
              ...
              <div className={[style['write-file-list'], style['ml-10']].join(' ')}>
                <ul>
                  <li>
                    {writerFileName ?
                        <h3>{writerFileName}</h3> : <h3>선택된 파일명</h3>
                    }
                  </li>
                  <li>
                    <h3>선택된 파일명</h3>
            ...
            <div className={[style['write-file-btn'], style['ml-20']].join(' ')}>
              ...
              <input type={'file'} id={'file-name'} className={style['write-file']} onChange={(e) => onWriteFileName(e)} accept={'image/*'}/>
              <label htmlFor={'file-name'} id={'file-name1'} className={style['file-name']}>파일 추가</label>
            </div>
          </div>
