...
const ProfilePage = () => {
    ...
    const reportChoice = (e) => {
        if(e.target.value === '비방 및 욕설' || e.target.value === '증오심 표현' || e.target.value === '성적 수치심을 일으키는 표현'){
                ...
                <div className={style['wrap']}>
                    <div className={style['profile-info']}>
                        <label>
                            <div className={style['profile-my-img']}>
                                <div className={style['profile-setup']}><img src={'setup.png'} alt={'설정'} onClick={() => navigate('/ProfileInfo')}/></div>
                            </div>
                        ...
                        <div className={style['follow-btn-wrap']}>
                            {follow ?
                                <button className={style['unfollow-btn']} onClick={() => unFollow()}>팔로잉</button> :
                                <button className={style['follow-btn']} onClick={() => Follow()}>팔로우</button>
                            }
                        </div>
                    </div>
                    {/*profile-info*/}
                    <div className={style['profile-board']}>
                        ...
                        <div className={style['profile-board-list-img']}>
                            <div className={style['profile-board-img-report']}>
                                <img className={style['img1']} src={'report-menu.png'} onClick={reportBtn} alt={'메뉴'}/>
                            {boardReport ?
                        ...
                        <div className={style['profile-board-icon']}>
                            <label>
                                <span><img src={'comment.png'} alt={'댓글'}/></span>
                                {!boardFavorite ?
                                    <span><img src={'favoriteDefault.png'} alt={'하트'} onClick={() => setBoardFavorite(!boardFavorite)}/></span> :
                                    <span><img src={'favorite.png'} alt={'하트'} onClick={() => setBoardFavorite(!boardFavorite)}/></span>
                                }
                            </label>
                        </div>
                    ...
                    <Modal isOpen={reportCheckModal} ariaHideApp={false} style={reportCheckModalStyle}>
                        <div className={style['report-modal-check-wrap']}>
                            ...
                            <div className={style['report-modal-check-text1']}>성한결 님을 신고하시겠습니까? </div>
                            <div className={style['report-modal-check-text2']}><span>신고는 철회할 수 없습니다.</span></div>
                            <div className={style['report-modal-check-button-wrap']}>
                                <button className={style['report-modal-check-report']} onClick={goReportCheck}>신고</button>
                                <button className={style['report-modal-check-cancel']} onClick={goReportCancel}>취소</button>
                            </div>
                        </div>
                    </Modal>
                    <Modal isOpen={reportCauseModal} ariaHideApp={false} style={reportCauseModalStyle}>
                        <div className={style['report-modal-cause-wrap']}>
                            <div className={style['report-modal-cause-title']}>
                                ...
                                <span className={style['close']}><img src={'close.png'} alt={'닫기'} onClick={goReportCauseCancel}/></span>
                            </div>
                            <div className={style['report-modal-cause-text']}>
                                <label>
                                    <span className={style['text']}>비방 및 욕설</span>
                                    <span><input type="radio" value={'비방 및 욕설'} onChange={(e) => reportChoice(e)} checked={reportCategory === '비방 및 욕설'}/></span>
                                </label>
                                <label>
                                    <span className={style['text']}>증오심 표현</span>
                                    <input type="radio" value={'증오심 표현'} onChange={(e) => reportChoice(e)} checked={reportCategory === '증오심 표현'}/>
                                </label>
                                <label>
                                    <span className={style['text']}>성적 수치심을 일으키는 표현</span>
                                    <input type="radio" value={'성적 수치심을 일으키는 표현'} onChange={(e) => reportChoice(e)} checked={reportCategory === '성적 수치심을 일으키는 표현'}/>
                                </label>
                                <div className={style['text-default']}>기타</div>
                                <textarea className={style['text-textarea']} value={reportCategoryText} onChange={(e) => reportChoice(e)}></textarea>
                            </div>

                            <div className={style['report-modal-cause-button-wrap']}>
                                <button className={style['report-modal-cause-close']} onClick={goReportCauseCancel}>취소</button>
                                <button className={style['report-modal-cause-report']} onClick={goReportCause}>신고</button>
                            </div>
                        </div>
                    </Modal>
                    <Modal isOpen={reportFinishModal} ariaHideApp={false} style={reportFinishModalStyle}>
                        <div className={style['report-modal-finish-wrap']}>
                            ...
                            <div className={style['report-modal-finish-text1']}>상대방은 회원님이 신고한 것을 알 수 없습니다.</div>
                            <div className={style['report-modal-finish-button-wrap']}>
                                <button className={style['report-modal-finish-report']} onClick={goReportFinish}>확인</button>
                            </div>
                        </div>
                    </Modal>
                            ...
                            <label>
                                ...
                                <div className={style['profile-comment-list-like']}>
                                    <div className={style['profile-comment-list-like-img']}>
                                        {!commentFavorite ?
                                            <span><img src={'favoriteDefault.png'} alt={'하트'} onClick={() => setCommentFavorite(!commentFavorite)}/>1,234</span> :
                                            <span><img src={'favorite.png'} alt={'하트'} onClick={() => setCommentFavorite(!commentFavorite)}/>1,234</span>
                                        }
                                    </div>
                                </div>
                            </label>
